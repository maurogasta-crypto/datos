...
| ~~D9~~ | casaverdecanas, CasaYourte | ~~Unificar el campo que lee `verItem`~~ | **Hecho el 2026-09-09.** Mauro eligió `permiso` —4 renglones contra 25, y queda la palabra entera—. Verificado leyendo el código de los dos el 2026-09-09 por la primera auditoría: `CasaYourte/nucleo.js:611` lee `it.permiso`, acepta lista vía `CY.puedeAlguno`, y **no queda un solo `perm` en todo el repositorio**. De paso CasaYourte ganó las listas de permisos, que no tenía. De acá salió la regla `general:llevar-no-reinventar` |
...
ni —desde el 2026-09-09— funciones de servidor: todo su estado vive en el
`localStorage` del teléfono. Por eso **no se agrega
...
> **El panel es el primero que la aplica** (sello `init-3`, 2026-09-09): el SDK
> entra por `import()` diferido dentro de un `try`, y si no baja la página abre
> y dice qué falta, con Reintentar. Lo que hizo el cambio barato fue exportar
> `let` en vez de `const` — un **enlace vivo**, que `cargarFirebase()` rellena,
> así que los treinta y pico de lugares que escriben `doc(db, …)` no se
> tocaron. **Los otros tres siguen pendientes, y pueden copiar el patrón.**
>
> **Y hasta ese día no lo aplicaba ninguno** — lo confirmó la primera
> auditoría leyendo los cuatro archivos. Los cuatro cargan Firebase con un
> `import` estático arriba de todo: `casaverdecanas/interno/firebase-init.js:8`,
> `CasaYourte/firebase-init.js:9`, `remate/interno/utils.js:87` y
> `datos/firebase-init.js:17`. La regla existe, es de ámbito `general`, está
> `vigente`, y no se cumple en ningún lado: es el caso de manual de una mejora
> que se quedó encerrada donde nació. Anotado como **A2** de
> `auditorias/2026-09-09-primera.md`.
>
> De paso apareció que **la versión del SDK diverge**: 12.16.0 en Casa Verde y
> CasaYourte, 10.12.0 en remate, 10.12.2 en el panel, sin nada que las compare
> (**A3**).
...
| Titularidad de las cuentas | ❌ — y **no es completable**: Mauro es *colaborador* de `toromboto/harmonia`, no dueño, así que no puede ver ni la instalación de la GitHub App ni sus autorizaciones. De ahí salió la decisión de la **incubadora**: este tipo de app se desarrolla en un repositorio propio de la cuenta de GitHub de Mauro (cuál, en `fichas/`) (`harmonia:H1`) |
| Etapa declarada (`PROTOCOLO-GENERAL.md` § 2.1) | ✅ — `Etapa: en desarrollo`, el primero del ecosistema en declararla |
