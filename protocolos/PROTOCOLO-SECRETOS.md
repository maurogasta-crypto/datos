...
## El agente entra a las bases: qué puede leer y qué no

Desde el **2026-09-11** un chat de Claude Code lee y escribe Firestore directo,
con `herramientas/firestore.mjs`. Antes generaba un JSON y Mauro lo pegaba a
mano. Tres reglas sostienen que eso no sea un agujero:

**1 · Entra como un usuario común, nunca con una cuenta de servicio.** Una
cuenta de servicio (Admin SDK) saltea todas las reglas de Firestore. Con ella
no habría bóveda — no por una decisión, sino porque las reglas dejarían de
aplicarse. El agente usa un usuario de Authentication, así que queda adentro
del mismo sistema de permisos que cualquier persona del equipo. **Y no se le
crea ficha en `usuarios/`:** no es del equipo, su acceso sale de una regla
propia.

**2 · Hay una bóveda por base, y se escribe en dos lugares.** Lo que el agente
no toca se anota en `selladas` (en la herramienta) **y** en la regla publicada
en la consola, en la misma tanda. La herramienta corta antes de salir a la red
y da un mensaje claro; la regla es lo que da la garantía. Una sola de las dos
no alcanza: si estuviera sólo en la herramienta, un error de tipeo abriría la
bóveda.

**3 · El sello es por lugar, no por contenido, y eso obliga a una disciplina de
guardado.** Ninguna regla puede adivinar que un campo llamado `valor` es una
contraseña. Una credencial va a la colección sellada y a ningún otro lado.

> **Ya falló una vez, el mismo día que se estrenó.** La primera verificación
> encontró en `fichas` —la colección de fichas técnicas del panel— un usuario y
> una contraseña reales de un servicio. Las reglas hicieron exactamente lo que
> decían hacer; el dato estaba en el lugar equivocado. Se selló `fichas`, se
> rotó la credencial y se sacó `respaldos/` del repositorio, porque el respaldo
> copia la base tal cual está y el historial de git es permanente.

**Qué se sella, como criterio:** credenciales de cualquier tipo, el libro del
negocio (dinero, honorarios, régimen impositivo) y los datos de personas
(huéspedes, clientes, reservas, mensajes). Lo que queda abierto es lo que sirve
para comparar la base con el código publicado: catálogo, contenido del sitio,
configuración pública. Ante la duda, se sella: sacar algo de la lista después
cuesta un renglón, y volver de una lectura que no debió pasar no cuesta nada
porque no se puede.

**El UID no se comparte entre proyectos.** El mismo mail y la misma contraseña
dan un UID distinto en cada proyecto de Firebase. El alta y la regla se hacen
una vez por base. El paso a paso está en `herramientas/ACCESO-A-LAS-BASES.md`.

**Y la contraseña del agente sigue siendo un secreto como cualquier otro:** la
carga Mauro en las variables de entorno de Claude Code, en la web, y ningún
chat la pide ni la escribe en ningún lado.
