...
## 12. El `package-lock.json`: qué cambia tenerlo y qué cambia no tenerlo

Escrito el 2026-09-09 porque Mauro lo pidió, respondiendo `harmonia:H5` en el
panel: «Preciso desarrollo de éste punto. No comprendo las implicaciones de la
decisión.» Acá está el desarrollo. **La decisión sigue abierta** — esto es para
poder tomarla, no la toma.

Aplica sólo a **Harmonía**, que es el único proyecto del ecosistema con `npm`.
Los otros cuatro no tienen build ni dependencias: no hay nada que fijar.

### 12.1 · Qué es, en una frase

`package.json` dice **qué rango** de versiones acepta el proyecto
(`"react": "^18.3.1"` = cualquier 18.x desde la 18.3.1). El
`package-lock.json` dice **exactamente qué versión** se instaló la última vez,
de cada paquete y de cada paquete de los que ésos usan — que en Harmonía son
unos cuantos cientos, no los ocho de `package.json`.

El primero es la intención; el segundo, el registro de lo que pasó.

### 12.2 · Lo que cambia de verdad

| | Con lockfile | Sin lockfile |
|---|---|---|
| **Cómo compila Vercel** | `npm ci`: instala exactamente lo del archivo, y **falla** si no coincide con `package.json` | `npm install`: resuelve los rangos al momento, y toma lo último que entre |
| **Dos builds del mismo commit** | dan lo mismo, hoy y en un año | pueden dar distinto, y no queda registro de en qué se diferenciaron |
| **Si una dependencia publica una versión rota** | no llega hasta que alguien actualice el lockfile a propósito | llega sola, en el próximo despliegue, sin que nadie haya tocado nada |
| **Si una dependencia se compromete** | igual: no entra sola | entra sola. Es el vector de la cadena de suministro |
| **Editar desde el teléfono** | el archivo se mueve solo cuando cambia una dependencia: son miles de líneas en un diff que no se puede leer en la web de GitHub | nunca aparece |
| **`git add -A`** | ya está commiteado; no sorprende | **se cuela**, que es exactamente lo que pasó el 2026-09-09 |
| **Actualizar algo** | hay que hacerlo a propósito (`npm update`) y commitear el archivo | pasa solo |

### 12.3 · El costo real de no tenerlo, dicho sin adornos

**No es hipotético: es la diferencia entre «el sitio dejó de andar» y «el sitio
dejó de andar y no sé qué cambió».** Sin lockfile, dos despliegues del mismo
commit pueden instalar versiones distintas. Si el de hoy compila y el de mañana
no, el commit es idéntico y no hay dónde mirar la diferencia — el registro de lo
que se instaló nunca existió.

Con lockfile, esa pregunta tiene respuesta: se compara el archivo contra el de
la última vez que anduvo, y ahí está.

### 12.4 · El costo real de tenerlo

**Un archivo grande que se mueve solo, en un proyecto que se edita desde el
teléfono.** No es una molestia estética:

- Un diff de miles de líneas en la web de GitHub no se puede revisar. Y § 2 de
  `PROTOCOLO-GENERAL.md` pide **mirar el contenido real de lo que se commitea**,
  no el nombre del archivo. Un archivo que en la práctica nadie puede mirar
  vuelve a esa regla un trámite.
- `npm ci` es más estricto: si `package.json` y el lockfile se desincronizan
  —porque alguien editó el primero desde la web de GitHub, que es cómo se
  trabaja acá— **el build falla**, donde antes habría seguido. Es la
  contrapartida directa de lo bueno: lo mismo que evita que entre una versión
  rota, evita que entre un cambio hecho a mano.

### 12.5 · La recomendación, y de qué depende

**Depende de la etapa** (`PROTOCOLO-GENERAL.md` § 2.1), y por eso no es una
regla sino una decisión:

- **En desarrollo** —donde está Harmonía hoy—, sin lockfile es defendible: no
  hay gente afuera a la que se le rompa nada, y editar desde el teléfono sin que
  aparezcan diffs ilegibles pesa más que la reproducibilidad.
- **Estable** —el día que Harmonía tenga usuarios—, el lockfile deja de ser
  opcional. Un sitio con gente adentro que no puede reproducir su propio build
  no tiene cómo volver atrás.

**Recomendación:** dejarlo afuera mientras la línea del `CLAUDE.md` diga
`Etapa: en desarrollo`, y agregarlo **en la misma tanda** en que esa línea pase
a `Etapa: estable`. Así la decisión no queda colgada de que alguien se acuerde:
queda atada a un cambio que ya hay que hacer.

**Y mientras tanto, que no entre por accidente.** Hasta el 2026-09-09 el
`.gitignore` de Harmonía **no lo bloqueaba** —se comprobó leyéndolo, no
suponiéndolo—, que es por qué se coló en un `git add -A`. Se agregó la línea ese
día, con el motivo escrito al lado y la instrucción de que adoptarlo es borrarla.
Así, que no esté es una decisión y no un olvido — y ésa es toda la diferencia
entre las dos cosas.
